...
TEST(AppendGeneratorTest, InfiniteSequenceBehavior) {
    ...
    EXPECT_EQ(gen.get_by_ordinal(Ordinal::omega()), 999);
    EXPECT_EQ(gen.get_by_ordinal(Ordinal(500)), 500);
...
TEST(AppendGeneratorTest, CloneBehavior) {
    ...
    EXPECT_EQ(static_cast<AppendGenerator<int>*>(clone)->get_by_ordinal(Ordinal(2)), 3);
